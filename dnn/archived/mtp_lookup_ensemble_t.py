...
def modelTargetPoisoning(model_p, logger, args):
    ...
    while condition:
        ...
        (x_opt, y_opt), best_loss = mtp_utils.lookup_based_optimal(
            theta_t=models_t, theta_p=model_p, loader=loader_optim,
            n_classes=ds_second.n_classes, random=args.random,
            lossfn=args.loss, filter=args.filter, verbose=True,
            ensemble_t=True, pick_optimal=args.pick_optimal)

        # Log some information about x*, y*
        with ch.no_grad():
            pred_p = model_p(x_opt)
            preds_t = [str(model_t(x_opt).argmax(1).item())
                       for model_t in models_t]
        print(utils.cyan_print("Loss: %.3f Mt(x*): %s, Mp(x*): %d, y*: %d" %
        ...
        for model_t in models_t:
            ...
            norm_diff = dnn_utils.model_l2_closeness(model_t, model_p)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_path', type=str,
                        default="./data/logs_pick_naive_ensemble",
    ...
    parser.add_argument('--seeds', default="80346,16,2021,4",
